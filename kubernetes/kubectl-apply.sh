...
logSummary() {
    echo ""
        ...
        echo "deploy - http://minikube_ip_placeholder:30200/"
        echo "#####################################################"
}
...
default() {
    ...
    kubectl create configmap minikube-cm  --from-literal=minikube-ip=$(kubectl get node -o jsonpath='{.items[0].status.addresses[?(@.type=="InternalIP")].address}') -n deploy
    kubectl apply -f deploy-${suffix}/
}
